...
def run_simulation(output_dir: str = ".", save_vtk: bool = True) -> dict:
    ...
    # 強制加振の設定
    # この系（質点1固定、m=1, k=100）の固有振動数:
    #   第1固有振動数: f1 = sqrt(150 - 50*sqrt(5)) / (2*pi) ≈ 0.984 Hz  ← 共振
    #   第2固有振動数: f2 = sqrt(150 + 50*sqrt(5)) / (2*pi) ≈ 2.576 Hz
    #   非共振の例: 1.354 Hz
    ampacc[IMASS] = 1.0
    freqacc[IMASS] = 0.5  # 0.5 Hz は第1固有振動数より低い非共振の例
